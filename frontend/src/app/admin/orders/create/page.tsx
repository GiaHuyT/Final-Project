...
import { ShoppingCart, Loader2, ArrowLeft, Phone, Mail } from 'lucide-react';
...
export default function AdminCreateInvoicePage() {
    ...
    const handleCreateInvoice = async () => {
        ...
        try {
            setIsCreating(true);
            await http.post('/orders/admin/manual', {
        ...
        <div className="container mx-auto py-10 max-w-4xl">
            ...
            <div className="bg-white rounded-3xl shadow-2xl p-10 border border-gray-100">
                <div className="flex items-center gap-5 mb-8 border-b border-gray-100 pb-8">
                    ...
                    <div>
                        <h1 className="text-3xl font-black text-gray-900 mb-2 uppercase tracking-tighter">Lên Hóa Đơn Hệ Thống</h1>
                        <p className="font-medium text-gray-500">Tạo hóa đơn bán xe trực tiếp bằng quyền Admin</p>
                    </div>
                </div>
                ...
                <div className="space-y-6 max-w-2xl">
                    ...
                    <div className="space-y-3">
                        ...
                        >
                            ...
                            {products.filter(p => p.status && p.stock > 0).map(p => (
                                <option key={p.id} value={p.id}>{p.name} (NCC: {p.vendor?.username || 'Chưa rõ'}) - Kho: {p.stock} chiếc</option>
                            ))}
                        </select>
                    </div>
                    ...
                    <div className="pt-6">
                        ...
                        >
                            {isCreating ? <Loader2 className="w-6 h-6 animate-spin mx-auto" /> : 'Phát Hành Hóa Đơn'}
                        </Button>
                    </div>
                </div>
            </div>
