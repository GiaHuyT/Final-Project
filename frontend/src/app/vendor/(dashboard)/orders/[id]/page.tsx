...
export default function VendorInvoiceDetailsPage() {
    ...
    useEffect(() => {
        const fetchOrder = async () => {
            try {
                // Actually, vendors might not have /orders/[id] yet, wait, does Vendor have a way to fetch 1 order?
                // The API is just GET /orders/:id which works for the authenticated user if they own it or if it's admin.
                // Assuming GET /orders/:id works for vendor if it's their order. Wait, the API for vendor is GET /orders/vendor/me which returns all. Let's just use it and filter, or if /orders/:id works, let's use it.
                // Wait, let's check backend/src/modules/orders/orders.controller.ts -> @Get(':id') is there. Does it check ownership?
        ...
        <div className="container mx-auto py-10 max-w-5xl">
            ...
            <div className="bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-100">
                <div className="bg-gradient-to-br from-blue-900 to-indigo-900 p-10 text-white relative overflow-hidden">
                    ...
                    <ShoppingBag className="w-12 h-12 mb-6 relative z-10 opacity-90 text-blue-300" />
                    ...
                    <p className="text-blue-100 font-medium relative z-10 text-lg">
                        Khách hàng: <span className="font-bold text-white">{order.customer?.username || 'Khách vãng lai'}</span>
                    </p>
                </div>
                ...
                <div className="p-10 bg-white">
                    ...
                    <div className="space-y-6">
                        {order.items?.map((item: any) => (
                            <div key={item.id} className="flex gap-6 p-6 bg-gray-50 rounded-2xl border border-gray-100">
                                ...
                                <div className="flex flex-col justify-between py-2 flex-1">
                                    <div>
                                        ...
                                        <p className="text-sm font-medium text-gray-500">Tình trạng: <span className="font-bold text-gray-700">{item.product?.condition || 'Xe mới'}</span></p>
                                    </div>
